...
class ChessDocumentRetriever:
    ...
    def get_context_for_query(self, query: str, max_chunks: int = 8, max_chars: int = 6000) -> Dict[str, Any]:
        """Get context for a query using environment variables for API keys"""
        try:
            import os
            ...
            if not openai_key:
                ...
                return {
                    "context": "No chess knowledge available due to missing API key.",
                    "context_count": 0,
                    "similarity_scores": []
                }
            
            if not qdrant_url or not qdrant_api_key:
                logger.warning("Qdrant credentials not found in environment")
                return {
                    "context": "Chess knowledge base is not available. Please ask general chess questions.",
                    "context_count": 0,
                    "similarity_scores": []
                }
            
            # Use async wrapper to call the async method
            import asyncio
            try:
                documents = asyncio.run(self.retrieve_documents(
                    query=query,
                    api_key=openai_key,
                ...
                if not documents:
                    return {
                        "context": "No relevant chess knowledge found for your query.",
                        "context_count": 0,
                        "similarity_scores": []
                    }
                ...
                for doc in documents:
                    ...
                    similarity_scores.append(doc.metadata.get('score', 0))
                ...
                logger.error(f"Error retrieving documents: {e}")
                return {
                    "context": "Chess knowledge base is temporarily unavailable.",
                    "context_count": 0,
                    "similarity_scores": []
                }
                
        except Exception as e:
            logger.error(f"Error in get_context_for_query: {e}")
            return {
                "context": "Error accessing chess knowledge base.",
                "context_count": 0,
                "similarity_scores": []
            }
...
# Initialize global document_retriever for backward compatibility
document_retriever = ChessDocumentRetriever() 